...
The platform uses a decoupled client-server architecture powered by a React client and an Express backend, backed by an ACID-compliant SQLite transaction database layer.
...
| :--- | :---: | :--- |
| **Role-Based Portals** | **Implemented** | Managed via JWT verification in [authMiddleware.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/authMiddleware.js) and role-specific client screens in [App.jsx](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/frontend/src/App.jsx). |
| **Admin Doctor Profile Control** | **Implemented** | Admin endpoints are managed in [admin.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/routes/admin.js) and rendered in [AdminPortal.jsx](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/frontend/src/portals/AdminPortal.jsx). |
| **Specialization Search & Slot Booking** | **Implemented** | Implemented search filtering by specialization and interactive slot booking in [PatientPortal.jsx](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/frontend/src/portals/PatientPortal.jsx) and the patient route [patient.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/routes/patient.js). |
| **Double-Booking & Mutex Checks** | **Implemented** | Database writes are serialized and locked transactionally in [bookingLock.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/bookingLock.js) to guarantee absolute concurrency protection. |
| **Doctor Leave Cancellations** | **Implemented** | Handled in the leave router endpoint in [admin.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/routes/admin.js#L126-L191). It cancels conflict bookings, deletes Google Calendar events, and sends email notifications. |
| **AI Pre-Visit Symptom Analysis** | **Implemented** | Symptoms are sent to Gemini in [llmService.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/llmService.js#L710-L802) to extract **Urgency Level**, **Chief Complaint**, and **Suggested Doctor Questions**. |
| **AI Post-Visit Summary** | **Implemented** | Triggers Gemini analysis in [llmService.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/llmService.js#L807-L926) to format a patient-friendly summary and split out individual medications. |
| **Medication Reminders Engine** | **Implemented** | Reminders are stored in `medication_reminders` and sent automatically by the background worker in [worker.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/worker.js). We also formatted frequency strings (`twice_daily` $\to$ `twice daily`) for premium readability. |
| **Email Notification System** | **Implemented** | Confirmations, cancellations, and reminders are queued and processed via [emailService.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/emailService.js) with support for both live SendGrid transmission and local `sent_emails.log` auditing. |
| **Google Calendar Integration** | **Implemented** | OAuth 2.0 calendar invites are created, synced, and deleted dynamically using [calendarService.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/calendarService.js). |
| **Graceful LLM Failure Handling** | **Implemented** | Wrap all Gemini calls in strict validations in [llmService.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/llmService.js). On API failures/rate limits, the server falls back to rule-based parsing in [llmService.js:L117-287](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/llmService.js#L117-L287) and [llmService.js:L292-705](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/llmService.js#L292-L705), populating `patient_summary` with the exact requested user-facing fallback message. |
...
## 6. Verification & Integration Test Suite

The platform includes an automated integration test suite located in [verify.js](file:///d:/HealthCare%20Appointment%201/HealthCare%20Appointment/backend/verify.js). This suite verifies the core concurrency safety systems, LLM fallbacks, and background worker systems:

1. **TEST 1: Concurrent Booking Race Condition Check**
   * Spawns parallel, concurrent booking tasks for the exact same doctor, date, and slot.
   * Asserts that the double-booking mutex lock correctly registers exactly one appointment and rejects the second with a concurrency error, preventing database race conditions.
2. **TEST 2: LLM Service Fallback Engine Check**
   * Simulates symptom reports under offline fallback mode.
   * Verifies that the rule-based backup engine correctly grades the symptom severity (e.g., "High" for chest pain and shortness of breath, "Low" for routine checks).
3. **TEST 3: Slot Hold and Expiry Check**
   * Tests the slot-hold locking engine. Ensures that once a slot is held by patient A, patient B cannot view or book it.
   * Asserts that after the hold expiration duration passes, the slot is correctly released and becomes available for other patients.
4. **TEST 4: Upcoming Appointment Reminders Check**
   * Injects an appointment scheduled for tomorrow and runs the worker scheduler.
   * Asserts that the background worker identifies the upcoming event, marks it as processed, and queues notification emails for both the patient and the doctor.
5. **TEST 5: Distributed Token Refresh Mutex Lock Check**
   * Runs concurrent OAuth refresh token requests in parallel.
   * Asserts that only one thread obtains the lock, and subsequent requests block and wait until the initial thread finishes refreshing the Google OAuth token.
6. **TEST 6: Doctor Deactivation and Cascading Cancellations Check**
   * Deactivates a doctor profile.
   * Verifies cascading cancellations of all active booked consultations under that doctor, along with cascade deletes of profile details across referenced tables.
...
## 7. Google Calendar Developer Setup
...
1. **Install Dependencies**
   ...
   npm run install:all
   ```
   *(Note: This runs installation scripts in both the `backend/` and `frontend/` directories from the monorepo root using the custom root script).*
